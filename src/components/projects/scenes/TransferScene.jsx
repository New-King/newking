...
function SendPanel({ phase, file, code }) {
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div
        className={`mt-3 flex h-[4.25rem] items-center rounded-lg border border-dashed px-3 transition-colors duration-300 sm:mt-4 ${
          showFile
      ...
      >
        {showFile ? (
          ...
          <p className="w-full text-center text-[10px] text-ink-faint">选择文件…</p>
        )}
      </div>
      ...
      <div className="mt-3 h-12 sm:mt-3.5">
        <div
          className={`h-full rounded-md border border-black/[0.08] bg-page px-3 py-2 text-center transition-opacity duration-300 dark:border-white/10 ${
...
function RetrievePanel({ retrieveStep, code, file, size }) {
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="mt-3 h-12 sm:mt-3.5">
        <div
          className={`flex h-full items-center justify-between rounded-md border border-black/[0.08] bg-page px-3 py-2 transition-opacity duration-300 dark:border-white/10 ${
...
export default function TransferScene({
  ...
  return (
    <div className="flex aspect-[16/9] items-center justify-center p-5 sm:p-8">
      <div className="relative h-[18.5rem] w-full max-w-[280px] sm:max-w-[300px]">
        <div
          className={`absolute inset-0 rounded-2xl border border-black/[0.08] bg-card px-5 py-5 shadow-apple transition-opacity duration-500 dark:border-white/10 sm:px-6 sm:py-6 ${
            isSend ? 'opacity-100' : 'pointer-events-none opacity-0'
          }`}
        >
        ...
        <div
          className={`absolute inset-0 rounded-2xl border border-black/[0.08] bg-card px-5 py-5 shadow-apple transition-opacity duration-500 dark:border-white/10 sm:px-6 sm:py-6 ${
            isSend ? 'pointer-events-none opacity-0' : 'opacity-100'
          }`}
        >
